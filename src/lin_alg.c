...
/*************************************************************************
 * LINEAR ALGEBRA ROUTINES FOR VECTORS
 *************************************************************************/
...
c_float vec_norm_inf(const c_float *a, size_t n) {
    ...
    const size_t remaining = n % block_size; /* cache-friendly implementation */
    ...
    switch (remaining) {
        ...
        case 1: max0 = max0 > c_absval(a[j+0]) ? max0 : c_absval(a[j]); __attribute__ ((fallthrough)); /* include remaining terms at the end */
        case 0:;
    }
    return max0;
}
