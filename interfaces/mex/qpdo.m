classdef qpdo < handle
    % qpdo interface class for QPDO solver
...
%         objectHandle % Handle to underlying C instance
   ...
   methods(Static)
        ...
        function out = constant(constant_name)
            ...
            out = qpdo_mex('constant', constant_name);
        end

    end
    methods
        %% Constructor - Create a new solver instance
        function this = qpdo(varargin)
            % Construct QPDO solver class
%             this.objectHandle = qpdo_mex('new', varargin{:});
        end

        %% Destructor - destroy the solver instance
        function delete(this)
            % Destroy QPDO solver class
            qpdo_mex('delete');
        end
        ...
        function varargout = setup(this, varargin)
            ...
            %
            % Convert infinity values to QPDO_INFINITY
            %
            bmin = max(bmin, -qpdo.constant('QPDO_INFTY'));
            bmax = min(bmax, +qpdo.constant('QPDO_INFTY'));
            ...
            [varargout{1:nargout}] = qpdo_mex('setup',n,m,Q,q,A,bmin,bmax,theSettings);
        ...
        function update_bounds(this, bmin, bmax)
            ...
            qpdo_mex('update_bounds', bmin, bmax);
        end
...
currentSettings = qpdo_mex('default_settings');
